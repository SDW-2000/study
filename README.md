...
이 버전으로 처음 시작하면 기존 메모 DB를 빈 본문이 가능한 구조로 자동 이전합니다. 회원이 있는 DB는 같은 디렉터리에 `*.before-api-v1-*.db` 백업을 먼저 생성하고 데이터를 보존합니다. Docker에서는 `/data` 볼륨에 보관됩니다. 앱 재시작 전 DB 복사본·저널을 위한 디스크 여유 공간을 확보하세요.
